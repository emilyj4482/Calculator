...
### 프로젝트를 통한 성장
#### 1. 예외 처리
> 정수형의 사칙연산이라는 간단한 기능만 구현하면 끝이라고 생각했던 과제가 알고 보니 버튼 종류에 따라, 눌린 버튼의 히스토리에 따라 다르게 처리해야 하는 예외 상황이 많다는 것을 알았습니다.
1. label text가 0인 상황에서 연산 버튼을 눌렀을 때
   - `-` 버튼의 경우는 음수 입력으로 인식하여 `0`을 대체하도록 처리
   - 나머지 연산자는 `0+`, `0*`와 같이 텍스트 누적 처리
2. 계산식이 완전하지 않은 상황에서 `=` 버튼이 눌렸을 때 : 무반응 처리
3. 연산자 버튼이 연속으로 눌렸을 때
   - 기본적으로 전에 눌린 연산자를 대체하도록 처리
   - `*` 또는 `/` 다음에 `-`가 눌렸을 경우에는 다음 숫자를 음수로 임력하는 것으로 인식하여 누적 처리
4. `0`으로 나눌 경우 label text가 `Error`라고 표시하도록 처리
5. `Error` 상황에서는 `AC` 버튼만 유효하고, 나머지 버튼들은 눌러도 무반응 처리
6. 연산자를 입력한 후 `0`이 두번 이상 눌리지 않도록 처리
7. 연산자를 입력한 후 `0`이 입력된 상태에서 `0`이 아닌 다른 숫자를 입력하면 `0`을 대체하도록 처리
#### 2. Subview와 ViewController의 역할
> 메인 view에 속한 컴포넌트들에 대한 커스텀 클래스를 정의했을 때, 뷰 클래스는 ui 구성과 관련된 내용만 가져야 하며, button action 호출과 같은 동작은 controller가 담당해야하는 것을 배웠습니다.
#### 3. MVVM 패턴 적용
> view를 구성하고 업데이트 하는데 필요한 데이터를 갖는 view model을 정의하고, 데이터를 다루는 service 객체와 통신하도록 구현했습니다. MVVM 패턴을 처음으로 스스로 구현해본 것이 패턴에 대한 이해도를 높이는데 큰 도움이 됐습니다.
